...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, mask_background, args, mask):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering_mask = render(view, gaussians, pipeline, background, override_color=mask.float().repeat(1, 3))["render"]
        # rendering_mask = torch.norm(rendering_mask, dim=0)
        rendering_mask = rendering_mask[[0]]
        rendering_mask = rendering_mask > 0.0  # 1, H, W
        ...
        rendered_mask_image[:, ~rendering_mask[0]] = rendered_mask_image[:, ~rendering_mask[0]] / 2
        rendering_mask = rendering_mask[None].float()
        gt = view.original_image[0:3, :, :]
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        torchvision.utils.save_image(rendering_mask, os.path.join(mask_render_path, '{0:05d}'.format(idx) + ".png"))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args):
    with torch.no_grad():
        ...
        # language_embeddings_path = os.path.join(dataset.model_path,
        #                     "point_cloud",
        #                     "iteration_" + str(30000),
        #                     f"language_embeddings.pt")
        # language_embeddings = torch.load(language_embeddings_path).cuda()
        masks_path = os.path.join(dataset.model_path,
                            "point_cloud",
                            "iteration_" + str(30000),
                            f"mask.pt")
        masks = torch.load(masks_path).cuda()
        ...
        # mask = clip.get_relevancy(language_embeddings, 0)[..., 0]
        # mask = relevancy > 0.5
        # if args.top_one:
